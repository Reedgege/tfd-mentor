...
跨平台构建脚本（Nuitka 版）
==========================
把论文格式医生·导师版编译为原生二进制「文件夹版」，再用 NSIS 打安装包（Windows）。
彻底规避 PyInstaller 单文件（onefile）运行时自解压导致的杀软误报。

  Windows -> dist/论文格式医生·导师版（免安装版）/  + installer.nsi -> tfd-mentor-setup.exe
  macOS   -> dist/tfd-mentor.app  (--macos-create-app-bundle)
  Linux   -> dist/tfd-mentor/
...
APP_NAME = "tfd-mentor"
# v1.0.32：Windows 分发名中文化——exe 带数字前缀排文件列表最前、dist 目录（即 zip
# 顶层文件夹）用中文产品名，客户解压后第一眼看到「1.论文格式医生·导师版.exe」。
# macOS/Linux 保持英文 APP_NAME（.app 包内/文件夹无"找 exe"问题，避免非 ASCII 路径 CI 风险）。
WIN_EXE_NAME = "1.论文格式医生·导师版"
WIN_DIR_NAME = "论文格式医生·导师版（免安装版）"
...
def _read_version():
    """读 VERSION 文件并去 'v' 前缀，返回纯数字版本（如 1.0.28）。"""
    try:
    ...
    if WIN:
        ...
        if _ver:
            opt += ["--company-name=" + COMPANY_NAME,
                    ...
                    "--file-description=" + PRODUCT_NAME + "（论文格式检查与修正工具）",
                    "--file-version=" + _ver,
