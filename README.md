...
**原生图形窗口（Tkinter），不依赖浏览器、零第三方依赖、论文不离开你的电脑**（联网仅用于「激活校验」与「首次试用登记」，只送激活码 / 机器码）。
