# -*- coding: utf-8 -*-
"""生成离线激活用的 RSA 密钥对（导师版独立一组，与学生版/海外版互不通用）。
...
产物（都在仓库根目录）：
    ...
    public_key.pem   —— 公钥，可入库作参考；真正使用的是嵌入进 tfd_app/crypto.py 的 PUBLIC_KEY_PEM 常量
...
def main():
    ...
    print("下一步：把 public_key.pem 的内容原样粘进 tfd_app/crypto.py 的 PUBLIC_KEY_PEM 常量。")
