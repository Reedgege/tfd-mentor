; 论文格式医生·导师版 · Windows 安装包 (NSIS)
; 把 Nuitka standalone 文件夹打包成单个 exe 安装器。
; 在仓库根目录运行：`makensis installer.nsi`（dist/tfd-mentor/ 须已存在）
; 版本号可用 `makensis /DVERSION=1.0.28 installer.nsi` 覆盖。
...
; （/DVERNUM=1.0.28），本地不带参数时为 0.0.0 占位，不影响构建。
!ifndef VERNUM
  !define VERNUM "0.0.0"
!endif
