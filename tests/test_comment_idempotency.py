...
注：导师版 run_fix_headings(modify=True) 为默认全改路径；本测试验证全改路径的
批注幂等。--no-modify（只批注不改）路径的批注幂等由同一 _apply_comments 去重分支覆盖。
