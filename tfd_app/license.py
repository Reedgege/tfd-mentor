...
本地授权文件：存于用户主目录 ~/.tfd_mentor_license/license.json，日常启动只读它，
             不联网，保证"论文处理全程离线"的卖点。
"""
import os
import sys
...
KAMI_APP = "daoshiban"       # 卡密通应用名（导师版专用，后台应用 daoshiban）
...
_OFFLINE_KEY = _obf("OykrMyIqITsgPTMgKSkjJiEqM31/fXkzPCYoITM5fQ==").encode("utf-8")
...
def _kami_http(card, machine_code, timeout):
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "tfd-mentor/1.0.0"})
...
def check_local_valid():
    ...
    导师版隔离：本机授权文件必须带 product=="mentor"，防止学生版授权
    （即便因误操作落入导师版目录）被误认，从源头杜绝两版授权互相通用。"""
    lic = load_local_license()
    if not lic:
        return False
    if lic.get("product") and lic.get("product") != "mentor":
