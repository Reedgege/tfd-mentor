...
def detect_heading(text):
    ...
    if re.match(r"^\d{1,3}(?:\.\d{1,3})+", t):
        ...
        if m:
            ...
            # 量级护栏（v1.1.20）：标题层级编号各段均为小整数（章≤50、节/子节≤99）。
            # 数据小数(如 80.92 / 1.533.59 / 91.37)首段或中段会很大，按"1.1"式标题匹配纯属
            # 误判，应归为正文——否则检查器会把表格数据当成"标题应为黑体"反复误报，
            # 且修正器也会试图给数字套标题样式。真实论文不会出现章>50或子节>99的编号。
            if nums and nums[0] <= 50 and all(n <= 99 for n in nums[1:]):
                dots = len(nums) - 1
                return (1 + dots, 0.95)
    else:
        # 单级数字：必须跟分隔符
...
def _looks_like_body_despite_heading_style(text):
    """标题识别为某级标题、但内容明显是正文时，应降级为正文（与 headings-fix 共用，
    确保检查器与修正器对"什么是标题"的判断完全一致，避免报告与修改明细对不上）。
    ...
    # 枚举列举项：数字+.+紧接着中文/字母（如「1.利用牌照优势…」「2.增加…」）是正文列举，
    # 绝非标题。数字后无空格、非多级编号（1.1）、直接接正文内容的，应降级为正文，
    # 否则检查器会把它当一级标题反复报"应为黑体"，修正器也会误套标题样式。
    ...
    if t[-1] in "。！？；.!?;":
        return True
    # 长度超阈值（纯编号/序数标题不受影响；第X章+论证短语已先走正文判定）
    if len(t) > 30:
        return True
    return False
...
def categorize_comment(text):
    ...
    两级匹配（v1.3.83）：
      1) 类别词出现在批注【开头 20 字符内】→ 直接归属。正常批注均为
         "类别词+要求"结构，类别词几乎总在开头；开头限定可排除说明性提及
         （如目录批注末尾"目录中的摘要，致谢，附录均不要空格"含"致谢/附录/
         摘要"字样却属 toc——v1.3.82 修复的误伤场景）。
      2) 开头无任何类别词命中 → 退回全文子串匹配。部分模板批注无类别词开头
         （如山大参考文献批注"黑体小三号…参考文献内容：…"，类别词在文本中部），
         v1.3.82 开头限定导致该类批注被误归 misc、参考文献 spec 缺失（用户
         实测"参考文献部分没改"的根因）；此时开头已无类别可被说明句误伤，
         全文匹配优于丢弃。
    """
    t = text or ""
    head = t[:20]
