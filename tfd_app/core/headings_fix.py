...
# 模板驱动修正开关：False 时 _format_runs/_set_para_format/_set_style 直接返回，
# 仅收集 changes 并写批注（导师版"只批注不改"模式由 --no-modify 触发）。
_MODIFY = True
...
def _resolve_heading_style(target, styles_map, lvl):
    ...
    根因修复（v1.1.24）：画像里的标题 styleId（如山大模板真实的 00001d / 000022 /
    ...
    if hspec.get("line_rule") or hspec.get("line_val"):
        ...
        if lr is not None and line is not None:
            ...
            if _cur_line is None or int(_cur_line) != line or (_cur_rule or "auto") != lr:
                return True
    # 加粗（v1.1.10：手动表单支持加粗；引擎原先不检查 bold，导致加粗要求不生效。
    # 仅当 hspec 显式要求加粗时才比对：逐 run 查 <w:b>，无 b 标记或 val=false/0 视为未加粗→需改）
    if hspec.get("bold"):
        _bold_ok = False
        for r in p.iter(WR + "r"):
            if r.find(WR + "commentReference") is not None:
...
def _initials_for(author):
    """批注作者缩写（Word 气泡小标识）。中文名取首两字即可。"""
    a = (author or "").strip()
...
def _apply_comments(z, root, changes, replacements, author=None):
    ...
    author: 批注署名（导师版可传导师名，如"论文格式医生·导师版"）。提示类批注
            自动追加"·提示"后缀，与"已修改"批注区分不同气泡颜色。
    ...
    for c in para_changes:
        ...
        # 提示类批注（只说明不修改：附录/关键词/疑似题注）用独立作者名（base+·提示），
        ...
        if _kind.endswith("_note") or _kind == "suspected_caption":
            ...
            add_comment_marker(p, next_id, text, comments_root,
                               author=base, initials=_initials_for(base))
        next_id += 1
        n += 1
...
def _build_report_data(applied, low_conf, skipped_residue, style_info, manual_items=None):
    """构造 (summary, columns, rows)，供 docx / xlsx 两种报告共用，保证格式一致。"""
    used_styles, template_exact, has_outline, tmpl_name, already_ok, comment_count = style_info
    all_c = applied + low_conf
...
def _build_change_docx(applied, low_conf, skipped_residue, style_info, dst_docx, manual_items=None):
    ...
    summary, columns, rows = _build_report_data(applied, low_conf, skipped_residue, style_info, manual_items)
    write_change_report(
        dst_docx,
...
def main():
    if len(sys.argv) < 3:
        print("用法: python headings-fix.py 输入.docx 输出.docx [--profile 模板画像.json] [--report 修改明细.docx] [--xlsx 修改明细.xlsx] [--author 批注署名] [--no-comments] [--no-modify]")
        sys.exit(1)
    ...
    global _MODIFY
    _MODIFY = "--no-modify" not in sys.argv
    author = None
    ...
    applied, low_conf, skipped, sinfo = fix(src, dst, profile, add_comments=add_comments, author=author)
    ...
    if xlsx_path:
        summary, columns, rows = _build_report_data(applied, low_conf, skipped, sinfo, manual_items=_manual_items)
        write_change_report_xlsx(xlsx_path, "论文格式修改明细（标题与正文）", summary, columns, rows)
        print(f"\n[已生成 Excel 修改明细] {xlsx_path}")
