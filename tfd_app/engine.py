# -*- coding: utf-8 -*-
"""
论文格式医生 · 导师版 引擎封装层（导师版内部使用，不直接给终端用户）
...
def run_fix_headings(src, dst, profile_path=None, report_docx=None, add_comments=True, modify=True, author=None, xlsx_path=None):
    """一键套标题样式（通用或模板驱动）。

    src: 输入 docx；dst: 输出 docx（新文件，不改动 src）；
    report_docx: 修改明细 docx（可选）；add_comments: 是否在文档写批注；
    xlsx_path: 修改明细 Excel（可选，导师版偏好）。返回 markdown 修改清单文本。
...
def run_annotate(src, dst, profile_path=None, author=None, xlsx_path=None):
    """导师版：只给问题段落加 Word 批注气泡、不改正文，生成可转发学生的批注副本。

    src: 学生论文 docx；dst: 输出 docx（原稿原样复制 + 批注）；profile_path: 学校模板画像；
    author: 批注署名（导师名）；xlsx_path: 修改明细 Excel（可选）。返回 markdown 修改清单文本。
    """
    return run_fix_headings(src, dst, profile_path=profile_path,
                           add_comments=True, modify=False, author=author, xlsx_path=xlsx_path)
