...
- **当前版本**：`VERSION` 文件 = `v1.1.2`（⚠️ 注意有 `v` 前缀，与学生版 `tfd-desktop` 的纯数字格式不同，别搞混）
...
|---|---|
| `main.py` | 程序入口 |
| `tfd_app/crypto.py` / `ed25519_verify.py` | 离线激活验签（Ed25519 纯标准库，零第三方加密依赖；私钥只在卖家本机 `_signing_keys/`） |
...
1. **论文绝对不上传**：本机处理。联网**仅**用于「在线激活校验」与「试用额度登记（只送机器码，绝不含论文内容）」。
